...
def haiku_select(
    ...
    """Call Haiku and return the selected tool name(s)."""
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:
                ...
                raise RuntimeError(f"Haiku API failed after {retries} tries: {exc}") from exc
    return ""
...
def run_condition(
    ...
    for i, q in enumerate(queries):
        raw = haiku_select(client, system_prompt, q["query"])
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"\nScale Benchmark — {len(QUERIES)} queries × {len(args.conditions)} conditions")
